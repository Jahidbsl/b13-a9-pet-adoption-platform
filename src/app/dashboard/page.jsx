...
const DashboardPage = () => {
  ...
  return (
    <div className="min-h-screen bg-[#FAF5FF] flex overflow-hidden relative">
      ...
      {/* Sidebar */}
      <aside
        className={`fixed lg:static top-0 left-0 z-50 h-screen w-[280px] bg-white border-r border-purple-100 shadow-2xl lg:shadow-none transition-all duration-300 ${
      ...
      <main className="flex-1 flex flex-col w-full">
        {/* Topbar */}
        <header className="sticky top-0 z-30 min-h-[90px] bg-white/70 backdrop-blur-xl border-b border-purple-100 px-4 md:px-6 lg:px-8 py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          {/* Left */}
          <div className="flex items-center gap-4">
            <button
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto">
            {/* Search */}
            <div className="flex items-center gap-3 bg-white border border-purple-100 rounded-2xl px-4 py-3 shadow-sm flex-1 sm:flex-none">
              <Search size={18} className="text-gray-400" />

              <input
                type="text"
                placeholder="Search pets..."
                className="outline-none bg-transparent text-sm w-full sm:w-44"
              />
            </div>

            {/* Notification */}
            <button className="relative min-w-[48px] h-12 rounded-2xl bg-[#F472B6]/10 flex items-center justify-center hover:scale-105 transition">
        ...
        <section className="p-4 md:p-6 lg:p-8 space-y-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 2xl:grid-cols-4 gap-6">
            {[
              ...
              <div
                key={idx}
                className={`bg-white rounded-[32px] p-6 shadow-lg hover:shadow-2xl transition border ${card.bg} relative overflow-hidden`}
              >
                <div
                ...
                <div className="relative">
                  ...
                  <h2 className="text-4xl md:text-5xl font-extrabold text-[#374151] mt-6">
                    {card.value}
                  </h2>

                  <p className="text-gray-500 mt-2">{card.title}</p>

                  <div className="flex items-center gap-2 mt-5 text-[#34D399] font-medium text-sm">
                    <TrendingUp size={18} />
                    Active this month
                  </div>
                </div>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
            ...
            <div className="xl:col-span-2 bg-white rounded-[32px] p-6 shadow-xl border border-purple-100">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
                ...
                <button className="bg-[#8B5CF6] hover:bg-[#7C3AED] text-white px-5 py-3 rounded-2xl font-medium transition w-full sm:w-auto">
                  View All
                </button>
              </div>
